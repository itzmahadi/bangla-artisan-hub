...
const Artisans = () => {
  ...
  const filteredArtisans = artisans.filter(artisan => {
    const matchesSearch = artisan.name.toLowerCase().includes(searchTerm.toLowerCase()) || artisan.craft.toLowerCase().includes(searchTerm.toLowerCase()) || artisan.location.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-green-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="md:col-span-2">
              <div className="relative">
                ...
                <Input type="text" placeholder="Search artisans..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 border-orange-200 focus:border-orange-400 bg-zinc-600" />
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredArtisans.map(artisan => <Card key={artisan.id} className="hover:shadow-xl transition-shadow border-orange-100 group">
              <div className="relative overflow-hidden">
                <img src={artisan.image} alt={artisan.name} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" />
                ...
                <div className="absolute top-4 right-4">
                  {artisan.verified && <Badge className="bg-green-500 text-white">
                      ✓ Verified
                    </Badge>}
                </div>
              </div>
              <CardContent className="p-6 bg-zinc-200">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button className="flex-1 bg-gradient-to-r from-orange-500 to-green-600 text-white hover:from-orange-600 hover:to-green-700">
                    Follow
                  </Button>
                </div>
              </CardContent>
        ...
        <div className="text-center mt-12">
          <Button variant="outline" size="lg" className="border-orange-200 text-orange-600 bg-teal-100">
            Load More Artisans
          </Button>
        </div>
      </div>
